...
public class D2CacheTransactionImpl implements D2CacheTransaction {
        ...
        if (traverse)
        {
            ...
            for (StoreTransaction txn : _storeTransactions) {
                if (txn.shouldStore(item.getStoreIn()))
                    recList.add(txn.addRecord(item.group(), keyList[0], item.getModified(), item.getTruth(), item.isNew()));
            }
            ...
            for (int i =1 ; i < keyList.length; i++) {
                for (StoreTransaction txn : _storeTransactions) {
                    if (txn.shouldStore(item.getStoreIn()))
                    {
                        StoreRecord rec = txn.addRecord(item.group(), keyList[i], item.getModified(), item.getTruth(), keyList[0], item.isNew());
            ...
            System.out.println("--->D2CacheTransactionImpl:"+item.getTruth()+"::"+item.getModified()+"::"+item.getOriginal() + ":" + item.isNew());
    ...
    public void waitToComplete() throws CtxException {
        for (StoreTransaction txn : _storeTransactions) {
            txn.waitToComplete();
        }
    }
