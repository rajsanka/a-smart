...
public class UploadManager {
	...
	public void handleDownloads(DownloadEvent u, SmartFileObject fOb)
        ...
        SmartData smo = (SmartData)fOb;
		TransactDSpace space = rshell.getSpaceFor(smo.smart___group());

		D2Cache fsCache = space.fsCacheImpl();

		Reader readr = fsCache.myReader();
		FileStoreReader fr = (FileStoreReader) readr;
		String flName = tenant.getName() + File.separator + smo.smart___group() + File.separator + fOb._fileName;
		assertion().assertNotNull( fr.getFileAsStream(flName,smo.smart___group() , null),"File does not exist on server");
		DownloadResponse r = new DownloadResponse(readr, fOb._size,flName,smo.smart___group(),fOb._fileType);
