...
public class HTTPClientObject implements PoolEntity
{
    ...
    public HTTPClientObject(ClientConfig cfg)
    {
        ...
        _httpconfig = new HTTPConfig("client", cfg.getPort(), cfg.useSecure());
        ...
        throws CtxException
    {
        ...
        {
            SCShell shell = null;
            if (SMCoreContext.coreContext() != null)
                shell = SMCoreContext.coreContext().smartChannelShell();
            else
                shell = _config.getShell();
            assertion().assertNotNull(shell, "Cannot create shell.");
    ...
    public Object postData(String uri, String post, boolean wait, Map<String, String> hdrs)
        throws CtxException
    {
        connect(wait);
        ByteArrayInputStream istr = new ByteArrayInputStream(post.getBytes());
        PData data = new GenericPData(null, new ContentData(istr));
        for (String nm : hdrs.keySet())
            ((GenericPData)data).addHeader(nm, hdrs.get(nm));
        _channel.post(uri, new PData[] { data });
        return handleResult(wait);
    }
    ...
    public String getFormattedObject(Object map, String format)
        throws CtxException
    {
        String formatter = _config.getFormatter(format);
        CrossLinkAny cl = new CrossLinkAny(formatter, this.getClass().getClassLoader());
        Object obj = cl.create(new Class[] { Object.class }, new Object[] { map });
        assertion().assertNotNull(obj, "Cannot create a formatter of type " + formatter);
        return obj.toString();
    }
