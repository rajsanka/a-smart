...
public class MonitorPlugin extends BasicPlugin implements Constants
{
        ...
        throws CtxException
    {
        ...
        String key = mtype.getMonitorKey(group, evtkey, parms);
        ...
        rshell.registerMonitorFor(group, mtype.dataClass());
        monsaved = (Map<String, Monitor>)ctx.getTxnSpecific(MONITORS);
        Monitor mon = monsaved.get(key);
        System.out.println("MonitorPlugin: Looking up : " + group + ":" + key + ":" + mon + ":");
        if (mon == null)
        {
            mon = (Monitor)rshell.lookupMonitorFor(group, key);
            if (mon != null) 
            {
                mon.smart___setIsNew(false);
                System.out.println("MonitorPlugin: Found up : " + group + ":" + key + ":" + mon + ":" + mon.smart___isNew());
            }
            /*try {
                //Development
        ...
        System.out.println("MonitorPlugin: Looked up : " + group + ":" + key + ":" + mon + ":");
        if (mon == null)
